...
try:
    from clinical_module.mrd.pipeline_executor_legacy import (
        FlowSOMPipeline,
        PipelineCheckpointManager,
    )
    _CLINICAL_AVAILABLE = True
except ImportError:
    FlowSOMPipeline = None  # type: ignore[assignment,misc]
...
class BatchPipeline:
    ...
    def _process_one(
        ...
        gating_logger = GatingLogger()
        _run_gating_ctx = GatingContext()  # contexte isolé par run, thread-safe
