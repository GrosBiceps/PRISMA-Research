...
class PreGating:
    ...
    def gate_singlets(
        ...
        if fsc_a_idx is None or fsc_h_idx is None:
            ...
            return GatingMask.all_true(n_cells)
    ...
    def gate_cd45_positive(
        ...
        return GatingMask(np.where(np.isnan(cd45_vals), False, cd45_vals > threshold))
    ...
    def gate_cd34_blasts(
        ...
        mask_cd34 = GatingMask(np.where(np.isnan(cd34_vals), False, cd34_vals >= threshold_cd34))
